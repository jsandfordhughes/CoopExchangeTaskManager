<template>
    <el-dialog v-model="dialogVisible" width="500">
        <template #header>
            <div v-if="editingTask">Edit task <span class="font-bold">{{form.title}}</span> </div>
        ...
        <el-form ref="formRef" :model="form" label-position="top" size="large" :rules="rules">
            ...
            <el-form-item label="Due Date" prop="due_date">
                <el-date-picker v-model="form.due_date" type="date" placeholder="Pick a date" class="!w-full"/>
            </el-form-item>
            <el-form-item label="Status" prop="status">
                <el-select v-model="form.status" placeholder="Select a status">
                    <el-option v-for="(label, value) in allowedStatuses" :key="value" :label="label" :value="value"/>
                </el-select>
            </el-form-item>
            <el-form-item label="Priority" prop="priority">
                <el-select v-model="form.priority" placeholder="Select a priority">
                    <el-option v-for="(label, value) in allowedPriorities" :key="value" :label="label" :value="value"/>
                </el-select>
            </el-form-item>
        </el-form>
        <template #footer>
            <div class="dialog-footer">
                <el-button :loading="form.processing" @click="handleSubmit" type="primary">
...
function handleSubmit() {
    formRef.value.validate((valid) => {
        ...
        const url = editingTask.value ? route('tasks.update', form.value.id) : route('tasks.store')
